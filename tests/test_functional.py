...
from torchaudio_contrib.functional import (stft, phase_vocoder, magphase, amplitude_to_db, db_to_amplitude, complex_norm, mu_law_encoding, mu_law_decoding
)

from test_layers import _num_stft_bins, _approx_all_equal, _all_equal
...
def test_stft(waveform, fft_len, hop_length, pad_mode):
    ...
    """
    pad = fft_len // 2
    window = torch.hann_window(fft_len)
    complex_spec = stft(waveform, fft_length=fft_len, hop_length=hop_length, window=window, pad_mode=pad_mode)
