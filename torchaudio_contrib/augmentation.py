...
class GaussianNoise(_ModuleNoStateBuffers):
    ...
    def forward(self, spect, scale=None):
        ...
        with torch.no_grad():
            ...
            noise = torch.normal(self.loc.expand(spect.shape), scale.expand(spect.shape)) 
            return spect, noise
...
def apply_masking(spect, max_value, mask_value, axis):
    ...
    mask = torch.arange(0, spect.size(axis)).repeat(spect.size(0), spect.size(1),1)
...
class FrequencyMasking(_AxisMasking):
    ...
    Args:
        max_freq (int): maximum possible length of the mask. Uniformly sampled from [0, max_freq).
        same_batch (bool): weather to apply the same mask to all the examples/channels in the batch. Defaults to False.
    """
...
class TimeMasking(_AxisMasking):
    ...
    Args:
        max_time (int): maximum possible length of the mask. Uniformly sampled from [0, max_time).
        same_batch (bool): weather to apply the same mask to all the examples/channels in the batch. Defaults to False.
    """
